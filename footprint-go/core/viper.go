...
import (
	"flag"
	"fmt"
	sysconf "github.com/fifpoet/footprint/config"
...
func InitializeViper(path ...string) *viper.Viper {
	...
	if len(path) == 0 {
		...
		if config == "" {
			...
			*/
			if configEnv := os.Getenv(sysconf.ConfigEnv); configEnv == "" {
				switch gin.Mode() {
				case gin.DebugMode:
					config = sysconf.ConfigDefaultFile
					fmt.Printf("您正在使用gin模式的%s环境名称,config的路径为%s\n", gin.EnvGinMode, sysconf.ConfigDefaultFile)
				case gin.ReleaseMode:
					config = sysconf.ConfigProdFile
					fmt.Printf("您正在使用gin模式的%s环境名称,config的路径为%s\n", gin.EnvGinMode, sysconf.ConfigProdFile)
				case gin.TestMode:
					config = sysconf.ConfigTestFile
					fmt.Printf("您正在使用gin模式的%s环境名称,config的路径为%s\n", gin.EnvGinMode, sysconf.ConfigTestFile)
				}
			} else {
				// internal.ConfigEnv 常量存储的环境变量不为空 将值赋值于config
				config = configEnv
				fmt.Printf("您正在使用%s环境变量,config的路径为%s\n", sysconf.ConfigEnv, config)
			}
		} else {
